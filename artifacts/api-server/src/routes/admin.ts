...
router.patch("/admin/users/:id/ban", requireAuth, requireAdmin, async (req, res): Promise<void> => {
  ...
  const [user] = await db.update(usersTable).set({ isBanned: true, isOnline: false, lastSeen: new Date() }).where(eq(usersTable.id, targetId)).returning();
...
router.patch("/admin/users/:id/unban", requireAuth, requireAdmin, async (req, res): Promise<void> => {
  const targetId = parseInt(req.params.id, 10);
  const [user] = await db.update(usersTable).set({ isBanned: false }).where(eq(usersTable.id, targetId)).returning();
  if (!user) { res.status(404).json({ error: "User not found" }); return; }
  res.json(formatAdminUser(user));
});
